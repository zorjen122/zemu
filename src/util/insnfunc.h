...
#define INSN_FUNCTION_H
#ifdef INSN_FUNCTION_H
...
#ifdef FLOAT_SORT_H_INCLUDE
#include "float.h"
#else
#include "interp_util.h"
#endif

//* The function in interp_utili.h is motivated by necessity and 
    // low performance loss of pure operators and operational functions.

static void func_empty(state_t* state, insn_t* insn) {}

#define FUNC(typ)                                                        \
    u64 addr = state->gp_regs[insn->rs1] + static_cast<i64>(insn->imm);  \
    state->gp_regs[insn->rd] = *reinterpret_cast<typ*>(TO_HOST(addr));   \
...
/* For Floating-Point Control and Status Registers,These function is not specifically implemented
  *  because there is no csr correlation defined here,
  *  so at present, the following is only applicable to the simulated user hierarchy.
  *  RISC-V control and status register (CSR) address map
  *     see reference RISC-V User-Level ISA V20180801-draft [charter 22]
  */
